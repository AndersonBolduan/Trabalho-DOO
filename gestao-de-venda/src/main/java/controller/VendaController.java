...
import model.util.AbstractMouseListener;
...
import model.entidades.VendaDetalhes;
import model.exception.NegocioException;
import model.util.VendaRegistroTableModel;
import model.util.VendaTableModel;
...
public class VendaController extends AbstractMouseListener implements ActionListener, KeyListener {
    ...
    public void keyReleased(KeyEvent ke) {
        String pesquisarProduto = this.dashboard.getTxtVendaPesquisarProduto().getText();
        String pesquisarClienteEusuario = this.dashboard.getTxtVendaPesquisar().getText();

        if (!pesquisarProduto.isEmpty()) {
            // Filtra produtos
            Optional<Produto> produtosTemp = produtos.stream()
                    .filter(p -> p.getId().toString().equals(pesquisarProduto)
                    || p.getNome().equalsIgnoreCase(pesquisarProduto))
                    .findFirst();
        ...
        } else if (!pesquisarClienteEusuario.isEmpty()) {
            // Filtra clientes e usuários
            List<Venda> vendasFiltradas = vendaDao.todosVendas().stream()
                    .filter(v -> {
                        return (v.getCliente() != null
                                && v.getCliente().getNome() != null
                                && v.getCliente().getNome().toLowerCase().contains(pesquisarClienteEusuario.toLowerCase()))
                                || (v.getUsuario() != null
                                && v.getUsuario().getNome() != null
                                && v.getUsuario().getNome().toLowerCase().contains(pesquisarClienteEusuario.toLowerCase()));
                    })
                    .collect(Collectors.toList());

            this.vendaTableModel = new VendaTableModel(vendasFiltradas);
            this.dashboard.getTabelaVenda().setModel(vendaTableModel);
            this.dashboard.getLabelHomeVenda().setText(String.format("%d", vendasFiltradas.size()));
        } else {
            // Caso todos os campos estejam vazios, atualiza com todas as vendas
            actualizarTabelaVenda();
        }
    }
    ...
    private void removerProdutoNoCesto() {
        if (this.nomeDoProduto != null && !this.nomeDoProduto.isEmpty()) {
            // Remover o produto do cesto
            this.vendaDetalhesCesto.remove(this.nomeDoProduto);

            // Recalcular o total da venda e o desconto
            double totalDesconto = this.vendaDetalhesCesto.values()
                    .stream()
                    .collect(Collectors.summingDouble(v -> v.getDesconto().doubleValue()));
            ...
            // Atualizar os campos na interface gráfica com os novos valores
            this.dashboard.getLabelVendaDesconto().setText(new BigDecimal(totalDesconto).toString());
            this.dashboard.getLabelVendaTotalDaVenda().setText(new BigDecimal(totalVenda).toString());

            // Atualizar a interface gráfica do cesto
    ...
    private void limparCesto() {
        vendaDetalhesCesto.clear();
        actualizarCesto(vendaDetalhesCesto);

        this.dashboard.getLabelVendaTotalDaVenda().setText("0.00");
        this.dashboard.getLabelVendaValorPago().setText("0.00");
        this.dashboard.getLabelVendaDesconto().setText("0.00");
        this.dashboard.getLabelVendaTroco().setText("0.00");

        mensagemNaTela("Cesto limpo com sucesso", Color.GREEN);

    }

    private void actualizarDesconto() {
        double totalDesconto = this.vendaDetalhesCesto.values()
                .stream()
                .collect(Collectors.summingDouble(v -> v.getDesconto().doubleValue()));
